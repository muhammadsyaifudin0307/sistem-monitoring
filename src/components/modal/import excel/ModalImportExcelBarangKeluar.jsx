...
const ModalImportExcelBarangKeluar = ({ isOpen, onClose, onImport }) => {
  ...
  const handleSubmit = () => {
    if (selectedFile) {
      onImport(selectedFile); // Panggil fungsi onImport dengan file yang dipilih
      setSelectedFile(null); // Reset state setelah impor selesai
      onClose(); // Tutup modal
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950 bg-opacity-50">
      <div className="bg-zinc-900 w-1/3 p-6 rounded shadow-lg">
        <h2 className="text-xl font-bold mb-4 text-zinc-100">
          Import Excel File Barang Keluar
        </h2>
        <input
          type="file"
          accept=".xlsx, .xls"
...
// Validasi tipe prop
ModalImportExcelBarangKeluar.propTypes = {
  isOpen: PropTypes.bool.isRequired,
  onClose: PropTypes.func.isRequired,
  onImport: PropTypes.func.isRequired,
};

export default ModalImportExcelBarangKeluar;
